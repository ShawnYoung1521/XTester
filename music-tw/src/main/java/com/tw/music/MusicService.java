...
public class MusicService extends Service {
    ...
    private boolean isPlayingbeforeACC = false;
    ...
    private Handler mHandler = new Handler(Looper.getMainLooper()) {
		...
		public void handleMessage(Message msg) {
			switch(msg.what) {
			case RETURN_ACCOFF:
				if (msg.arg1 == 3 && msg.arg2 == 1) { //待机
					isSleep = true;
					save();
					if (isPlaying()) {
						pause();
						isPlayingbeforeACC = true;
					}else{
						isPlayingbeforeACC = false;
					}
				}
				if (msg.arg1 == 3 && msg.arg2 == 0) { //正常
					isSleep = false;
					if(isPlayingbeforeACC){
						start();					
						isPlayingbeforeACC = false;
					}
				}
			...
			case SHOW_PROGRESS:
				...
				break;
			case TWMusic.BT_CALL_STATE:
				if (mSource!=0x03) {
					return;
				}
				int state=msg.arg1;
				Log.d(TAG, "BT_CALL STATE:"+state);
				switch (state) {
				case 0://通话结束、挂断
					callState=false;
					if (prevStateIsPlaying&&!isSleep) {
						start();
					}
					break;
				case 1://通话中
	...
	private boolean prevStateIsPlaying=false;
	private boolean callState=false;//蓝牙通话的状态返回都是只返回0x00和0x01，所以需要用这个来标记当前状态
	@Override
	...
	public void stop() {
		// mTW.mCurrentPos = mMediaPlayer.getCurrentPosition(); //deleted by XY
		mMediaPlayer.stop();
		mHandler.removeMessages(SHOW_PROGRESS);
